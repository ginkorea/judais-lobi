...
def _main(AgentClass):
    ...
    parser.add_argument("--cognition", action="store_true",
                        ...
                        help="Carry a shadow cognitive state through this "
                             ...
                             "events.jsonl. SHADOW means it never decides: "
                             "no gate, no answer held, checked or refused "
                             "against it, and with it off the run is byte "
                             "for byte the run it always was. One thing "
                             "reads it back, and only where the run has "
                             "GOALS loaded (a rule pack): at each step "
                             "boundary the supervisor is handed what the "
                             "run believes, and a frontier that has not "
                             "moved for five steps may spend one advisory "
                             "review turn the run would not otherwise have "
                             "spent. That review can nudge and it can never "
                             "end the mission. No goals, no signal, and "
                             "--compiled-context is the flag that puts any "
                             "of this in front of the model. It is not free "
                             "of TIME, though: "
    ...
    parser.add_argument("--compiled-context", action="store_true",
                        ...
                        help="Put the runtime's view of the problem into "
                             ...
                             "what the goals still require (OWED — the "
                             "ranked frontier, cheapest first, naming the "
                             "goal each line serves), and what is only a "
                             "model's claim — each graded "
                             "verified / sourced / model-extracted / "
                             "speculative, and a line the store CONCLUDED "
                             "rather than read marked `derived` because no "
                             "receipt stands behind it. It rides last, next to the "
