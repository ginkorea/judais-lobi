...
| `--no-task-state` | — | opt out of default run-local task/reference tracking; incompatible with scoped task flags. Resume/replay retain their recorded mode |
| `--task-owner` | — | caller-supplied owner binding for private state, paired with `--task-thread`; not authentication or a permission grant |
| `--task-thread` | — | caller-supplied private thread binding, paired with `--task-owner`; obtain both from application state, never conversation text |
| `--task-state-in` | — | private prior-turn pointer file, requiring both scope flags; resolves through the same host's durable `JUDAIS_LOBI_RUNS` store |
| `--task-state-out` | — | private output pointer file written at finalization, requiring both scope flags; use one path per turn. See [current task state](docs/current-task-state.md) |
...
| `--defer-skills` | — | direct missions: begin with a compact configured-skill index and load full instructions, schemas and tool selection through `select_skills`; does not widen permissions or bypass gates. Swarm, campaign and recorded replay combinations are unsupported |
| `--active-skill` | — | restore a configured skill ID selected in this conversation; repeatable and requires `--defer-skills`. Restores selection, not permissions or approvals; resume retains the recorded selection |
| `--cognition` | `JUDAIS_LOBI_COGNITION` | carry a **shadow** cognitive state through the mission: every tool receipt is harvested into propositions whose evidence is the receipt, and the kernel's event log is written as `reasoning.jsonl` beside the run's `events.jsonl`. Nothing is gated — no answer is held, checked or refused against it — and with it off the run is byte for byte what it always was. One thing reads it back, and only where the run has **goals** loaded: the supervisor is handed what the run believes at each step boundary, and a frontier that has not moved for five steps may spend one advisory review the run would not otherwise have spent. That review can nudge, and it can never end the mission. No goals, no signal. Needs a run directory (`JUDAIS_LOBI_RUNS` on) |
| — (a manifest field) | — | **`cognition:` — the rule pack.** A `SKILL.md` may carry the clauses a run reasons under: `cardinality` (a field carries `one` value or `many`; `many` unless declared, because a false conflict is destructive), `rules` (positive horn clauses over `["?a", "field", "?v"]` patterns, loaded at `SKILL` authority through the kernel's propose-then-promote door), `goals` (target patterns — every premise the store cannot satisfy becomes an obligation), and `constraints` (arithmetic that must hold over the facts — `require: "settled + pending == total"`, decided in exact rational arithmetic with no new dependency; `require_z3:` is the opt-in escalation for division and non-linear products, and the `judais-lobi[solver]` extra buys **checking, never loadability** — a manifest that writes one loads everywhere, and without the extra those constraints go unchecked with one note in the log). With `--cognition` the pack loads before the first receipt, so receipts **derive** and the frontier says what is still owed; without it the block is still parsed — an unusable one refuses at the door — and does nothing. It can never gate: a constraint that does not hold is a line in the view and a record in `reasoning.jsonl`, never an answer held or a mission ended. Several skills' packs union by name. See `PLATFORMS.md` |
| `--compiled-context` | `JUDAIS_LOBI_COMPILED_CONTEXT` | put the runtime's **view of the problem** into each step's model input: one block, replacing the one before it, with the established facts as `entity · field = value` lines carrying the receipt handle each came from, both sides of every open conflict, **what the goals still require** (`OWED` — the ranked frontier, cheapest first, each line naming the goal it serves and what blocks it), and what is only a model's claim — each graded `verified` / `sourced` / `model-extracted` / `speculative`, with a line the store *concluded* rather than read marked `derived`, because no receipt stands behind it. Rides last, next to the current turn, so the cached prefix does not move; bounded to 4,000 characters, and what does not fit is named rather than dropped in silence. **Implies `--cognition`** and gates nothing: no answer is held, checked or refused against it |
| `--swarm-steering` | `JUDAIS_LOBI_SWARM_STEERING` | offer a **staged** turn's planner the **independent groups** of what the run still owes: the proof frontier partitioned by what the store's own links say is one subject, so obligations sharing no subject fall in different groups. One bounded block per planning round (4 groups, 3 owed lines each, both overflows named), in the same `owed:` spelling `--compiled-context` uses. **Implies `--cognition`** and needs the same run directory. Nothing new on the wire — what changes is one planning prompt — and it decides nothing: no child is forced per group, no plan is refused for ignoring it, nothing reaches the supervisor |
| `--graph-context` | `JUDAIS_LOBI_GRAPH_CONTEXT` | keep this run's links as a **topology** beside the store — `graph.jsonl` in the run directory, its own versioned header — and add one **RELATED** section to the compiled block: the entities a bounded walk out from the `OWED` lines reaches, one edge to a line, each naming the relation and the authority it arrived on. Nothing in it is guessed — every edge is one of the store's own links, carrying that link's evidence — so a line is a *reason to look* and never a finding, which is why it is the first section the budget drops. No goals, nothing owed, no section. **Implies `--compiled-context`** (and through it `--cognition`); gates nothing, and a graph that fails costs the section, writes one note, and leaves the mission exactly as it was |
...
| `JUDAIS_LOBI_MAX_OUTPUT_TOKENS` | explicit default completion ceiling for the local backend; takes precedence over the output profile. Blank, unparseable or non-positive falls back to the profile. A completion that hits the ceiling says so as `usage.finish_reason` when usage is available |
| `JUDAIS_LOBI_OUTPUT_PROFILE` | implicit local-backend output target: `standard` (8,192; default), `synthesis` (16,384) or `extended` (32,768), capped at half a known context window to leave input space. Unknown values use `standard`; explicit operator/call ceilings take precedence |
...
| 1.5.0 | 14 Sep 2026 | **the subject spine, and Phase 20 complete**. What a plane's tools say about themselves is read once (`outputSchema` `x-` verbs win, the manifest `tools:` block is the fallback, every disagreement a named discrepancy); a declared identifier is harvested, the receipt links to its subject at `source`, and the kernel projects receipts together at the subject — so two tools naming one job finally join, contest only where they truly disagree, and the compiled view folds a subject's facts into one line naming its calls. `--graph-context` keeps the links as a topology with a RELATED section that drops first; `--swarm-steering` offers a staged planner the frontier's independent groups, advisory forever; rule packs gain `constraints:` in exact `Fraction` arithmetic with the solver as an optional extra that buys checking, never loadability. A slow answer becomes a fact on the wire (`model_state: streaming`) and an unbounded one becomes a bounded, reported truncation (`max_tokens` 4096 by default, `usage.finish_reason`). The measurement rides along: `core.eval context` (chars per call beside every capability delta), `suggest-pack` (a draft pack a person reviews), `linker` (false-link rate, shipped 0), the five-arm attribution table with its reading rules, and spend columns defined against the declared obligation path |
| 1.4.0 | 14 Sep 2026 | **the frontier is real** (Phase 19, plus the first Phase 20/21 pieces). Rule packs arrive through skill manifests — `cognition:` blocks carrying cardinality, rules at skill authority, and goals, door-validated with a kernel dry-run and composed like everything else — so a mission derives facts no receipt stated and owes obligations. The compiled view gains OWED (the ranked frontier; owed outlives facts because a dropped obligation is reachable from nowhere) and marks derived lines. The supervisor watches belief as well as behaviour: a frontier frozen for five steps earns one advisory review that is constitutionally incapable of ending a mission. `core.eval registry` (measured model profiles, one writer, no cross-interpreter averaging) and `core.eval corpus` (validated traces to fine-tune examples, verbatim completions, abstention-heavy). First A/B in evidence: constrained decoding is a null result on the reference 20b, as predicted |
| 1.3.1 | 14 Sep 2026 | a number carries its interpreter, an error names its cause: the `infra` class in every eval report (a run that never reached a model measured the environment — its own column, out of the denominator, never silent), the MCP handshake timeout named in its own refusal with one owner, the conformance kit's green line says what it compared (`KIT_VERSION`), and one Wilson — with the intervals that moved in the fourth decimal pinned and disclosed |
| 1.3.0 | 14 Sep 2026 | **the view and the grammar** (Phase 18, same day). `--compiled-context` (implies `--cognition`): the runtime's view of the problem rides in each step's model input — one budgeted block, replaced each step, holding established facts with the receipt handle and confidence band on every line (verified / sourced / model-extracted / speculative, `ungraded` distinct, `disputed` marked), both sides of every open conflict, truncation always announced with the escape pointing at the result store eviction cannot reach; it can never gate, and the cut is chosen arithmetically, rendered once, and brute-force-oracle-verified. `supports_json_schema` as a declared backend capability (honest per provider) and `core.eval extraction --constrained` for the structural-validity A/B; the scorer's version joins the experiment's identity. `--version` says what is deployed, never what the source claims. First measured evidence in-tree: `evidence/extraction/` — the reference platform's 20b at 92% probe-reliable extraction with zero fabricated citations |
| 1.2.0 | 14 Sep 2026 | **the cognitive core** (ROADMAP §2.9, built in one night of parallel lanes under adversarial review). `core/cognition`: the epistemic kernel — propositions carrying status, authority and evidence; trusted-rule closure with exact provenance (`prove()` returns the DAG to the receipts); contradictions first-class with per-field cardinality and evidenced settlement; obligations computed from goals; a confidence API (`support`/`summarize`); an event-log that replays bit-exact and refuses corruption including tail cuts. `core/cognition/graph`: the knowledge graph — authority-carrying edges (a model-inferred edge is never confused with a structural one), budgeted working-set `hydrate` with truncation never silent. `--cognition`: shadow attachment — receipts harvested deterministically into the kernel, `reasoning.jsonl` beside the stream; off is byte-identical, on never blocks an answer, and its wall-clock cost is stated, not hidden. `core.eval extraction`: the semantic-reliability instrument (real-receipt probes; the headline is bound to honest citations). The 12-mission benchmark pack and `core.eval ablation` (arms as flag-deltas; an arm graduates the day its flag lands, with no edit). One wire change: the `--cognition` flag and its env var |
| 1.1.1 | 26 Aug 2026 | a reference platform's first day composing: a grounding key declared by every skill but empty no longer vanishes from the merged block (composition may change a value, never a declared key's presence — `GROUNDING_KEYS` now has a guard that fails the day a key is added without a merge rule); the conformance kit's checkout locator walks ancestors, so a lane in a git worktree finds the real validator instead of silently skipping it, and a repository that is itself a checkout beats its ancestors; `PLATFORMS.md` names `JUDAIS_LOBI_HOME` and says `.output_contract` where an implementer once read `output_format` |
| 1.1.0 | 25 Aug 2026 | **skills compose**. `--skill` repeats (and `MISSION_SKILL` splits on the path separator; a typed flag wins); several manifests become one mission: the closed sets union (first-seen order, required-anywhere binds), only the first skill's `output_format` is the answer's, and the grounding blocks merge rather than being picked from — lists union, a plane restated in another order or spelling is one plane while a family (`x_*`) and a single tool are two, booleans OR, a named `must_cite` minimum cannot dig under another skill's wildcard floor, and scalars that disagree are a refusal naming both skills. Every gate is order-independent: the code-plane sandbox declaration is asked of each manifest under the composed isolation. `compose_manifests` is on the library façade; a composing spawn line in `core.eval measure` runs its untiered rows and reports the tiered ones as unmeasurable rather than half-measuring them. One wire note: `meta.json` `flags.skill` is now a list (was a string) |
| 1.0.0 | 21 Aug 2026 | **the initial release**. The freeze (18 Aug): `SCHEMA_VERSION` 1 frozen for the 1.x major (`CONTRACT.md` §1.0); the conduct text rendered once; `mission_result` pages text results; figures ground where they were measured (`figures_from:`); a mission has a root; the sandbox runner is carried per call; liveness is a lock, not a clock; the supervisor treats a new call OR a new result as progress; `--mission-steps 0` is no ceiling. The release-candidate iteration on a reference platform (18–21 Aug): three conduct sentences (a derived figure is a computation tool's to print; a listable reference is looked up before the human is asked; candidates for a state-changing act are chosen out loud) and two error-recovery bounds; the conduct renders after the catalogue; `--mcp-timeout`/`MCP_TIMEOUT_S`; the critique names the fix (repair turns steer to the code plane), and two grounding checks — `attribution` (a figure's named field must exist and hold it) and `subject` (an answer about the run's own tooling is not an answer) |
