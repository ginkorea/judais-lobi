...
| **`tool_result`** | `call`, `quoted_history` | `call` pairs the result with its tool call; `quoted_history: true` marks a conversation-history read (including a reread), not verified evidence or new authorization |
