...
| **`tool_result`** | `call`, `quoted_history`, `receipt`, `redacted_receipt` | `call` pairs the result with its tool call; `quoted_history: true` marks a conversation-history read (including a reread), not verified evidence or new authorization. `receipt` optionally references a private v1 checkpoint by status/id/digest/byte count/redaction flag; no payload or filesystem path travels in it. `redacted_receipt: true` preserves redacted-source provenance through local rereads |
...
| **`mission_finished`** | `usage`, `telemetry`, `budget`, `reason`, `elapsed_s`, `stopped_with_draft`, `task_state` | the run's ledger and optional observation coverage; which budget ran out and by how much; why it ended when the outcome word does not say; the wall clock; whether the run ended early having handed over an answer it had already written; optional measured current-task coverage (see [current task state](docs/current-task-state.md)) |
...
A run with `--cognition` on adds one more, `frozen_frontier`: five steps in
which the proof frontier did not move, no contradiction was reduced and almost
nothing new was established. It is the one signal that is not about what the
run *did*, it never fires where nothing is owed, and it changes nothing here —
the same `review` field, the same four verdicts, no new record and no new
field. It is also the one signal that **cannot end a run**: its review is never
offered `stuck`, so `mission_finished.reason` is never `"stuck"` because of it. A consumer that has never heard of it reads it as it reads any signal
name: `signal` is not a closed set, which this document already says.
...
- `--defer-skills` — direct missions only: expose an index of configured skills and their tool names first. The mission calls the internal `select_skills` tool with one or several configured IDs to load their full instructions and schemas, and may replace that selection later. Hidden tools cannot execute; newly selected tools wait for the next model turn. The complete configured manifests still set the unchanged permissions and sandbox ceiling; evidence checks accumulate across selected skills and direct resumes. Resume must retain the recorded loading mode. Swarm, campaign and recorded-replay combinations are not yet supported and refuse before execution. `--no-grounding`, when explicitly selected by the operator, still disables those checks.
...
- `--cognition` — carry a **shadow** cognitive state through the run: every tool receipt is harvested into propositions whose evidence is the receipt, and the kernel's whole event log is written as `reasoning.jsonl` in the run directory beside `events.jsonl`, `model.jsonl` and `tools.jsonl`, under the same `JUDAIS_LOBI_RUNS`. It adds **nothing to this contract**: no record type, no field, no outcome word, no new outcome, and no gate on an answer. With it off a run is byte for byte the run it was before the flag existed. With it on and **no goals loaded** — which is every run until a rule pack gives it some — the only difference in what is recorded is that file. With it on and goals loaded, the supervisor gains one more signal, `frozen_frontier` (see `review` above): a run whose proof frontier stops moving may spend one advisory review turn it would not otherwise have spent, which rides the `review` field that already exists, carries no new verdict, and cannot end a run at all — the review is never offered `stuck`, and where the review budget is spent it produces no review rather than a wind-up. It is not free of **time**: the harvest and one `fsync` run inside the step, on the loop's own thread, at roughly 6 ms a step on the machine this was measured on — noise beside a model call, a real number beside a tight `--mission-seconds`. "Shadow" is a claim about gating, not about the clock: nothing here is ever waited on for permission, and no answer is held, changed or refused because of it. It needs a run directory — with the store off there is nowhere to write it and the harness says so. `reasoning.jsonl` carries three version numbers of its own — the file's shape, the kernel's event vocabulary and the kernel engine that assigned the ids — and none of them is `schema_version`.
- `--compiled-context` — put the runtime's **view of the problem** into each step's model input: one block, replacing the block the step before it had, holding the established facts as `entity · field = value` lines with the receipt handle each came from, both sides of every open conflict — including, where the skill's pack declares `constraints:`, a `constraint:` line naming a declared arithmetic that does not hold over the facts, which is recorded and advisory like everything else in the block — and what is only a model's claim — each graded `verified` / `sourced` / `model-extracted` / `speculative`, marked `disputed` where something contradicts it, and marked `derived` where the runtime *concluded* the line rather than read it — a derived line's receipts are its premises', so the handle it prints is not one a model may cite as a source. Where the run has goals loaded it also carries **OWED**: the proof frontier, one line per unresolved obligation — the pattern that is missing, the goal it serves, and whether it is open or blocked and on how many — ranked cheapest-first, and saying so when the frontier walk itself stopped short. Those lines are **state, not instruction**: they say what is missing and never what to call. It **implies `--cognition`** and turns it on rather than refusing, because the view is compiled from that state; it therefore needs the same run directory. It adds **nothing to this contract**: no record type, no field, no outcome word, nothing new on the wire. What changes is the model's *input*, which is visible only in the run's own `model.jsonl` — a consumer comparing recorded prompts across a pin should expect a run with this on to have asked different questions than the same run with it off, which is the point of the flag and the reason it is off by default. It **gates nothing**: no answer is held, checked or refused against the block, and a compiler that fails stops compiling, writes one note into `reasoning.jsonl` and leaves the mission exactly as it was. The block rides **last**, next to the current turn, so the cached prefix (persona, protocol, catalogue, conduct) does not move for it; it is bounded to 4,000 characters, and what does not fit is named in the block rather than dropped in silence, pointing at the mission's **result store** — the block is itself window pressure, so a promise that the receipts are still in the transcript is one the block can falsify, while the store holds every receipt whole under the same handle the fact lines print.
- `--swarm-steering` — offer a **staged** turn's planner the **independent groups** of what this run still owes: the proof frontier partitioned by what the store's own links say is the same subject, so two obligations that share no subject fall in different groups and two that a link joins fall in one. One bounded block per planning round — capped at 4 groups and 3 owed lines each, with both overflows named, and saying so when the frontier walk was itself cut short, because an obligation the walk never reached could have been the one that joined two of the groups shown. The owed lines are the same lines `--compiled-context`'s **OWED** section prints. It is **advisory and there is no second half**: the planner reads it and plans, no child is forced per group, no plan is refused for ignoring it, nothing reaches the supervisor, and the plan validator has never heard of a group. It **implies `--cognition`** and turns it on rather than refusing, because the groups are computed from that state; it therefore needs the same run directory. It adds **nothing to this contract**: no record type, no field, no outcome word, nothing new on the wire. What changes is one planning prompt, visible only in the run's own `model.jsonl` — and only for a turn that staged at all and whose frontier has at least two independent groups; every other run asks the question it would have asked with the flag off, byte for byte. A hint that fails stops for the run, writes one note into `reasoning.jsonl` and leaves the mission exactly as it was.
- `--graph-context` — keep this run's links as a **topology** beside the store, and show the model what it connects to what is owed. Two additions and no more. First, `graph.jsonl` in the run directory beside `reasoning.jsonl`, under the same `JUDAIS_LOBI_RUNS`: the store's own links — a receipt entity *about* a subject, made deterministically from a platform's declaration — written as graph edges carrying each link's own authority and evidence, with a versioned header of its own (the file's shape, the graph's event vocabulary, and the graph package that assigned the edge ids — none of them `schema_version`). Nothing in it is guessed: no model proposal and no value coincidence can enter, so an edge is a *reason to look* and never a finding. Second, the compiled block gains one **RELATED** section: the entities a bounded walk of that topology reaches out from the OWED lines — hydrated around what is owed, so a run with no goals loaded has no section at all — one edge to a line, each naming the relation and the authority it arrived on, capped, budget-participating, and the **first** section the budget drops. It **implies `--compiled-context`** (and through it `--cognition`) and turns both on rather than refusing, because RELATED is a section of that block. It adds **nothing to this contract**: no record type, no field, no outcome word, nothing new on the wire — one more file a run leaves behind, and a change to the model's *input* visible only in `model.jsonl`, with the same warning to a consumer comparing recorded prompts. It **gates nothing**: a graph that fails — harvest, log or walk — costs the topology and the section, writes one note into `reasoning.jsonl`, and leaves the harvest, the view and the mission exactly as they were.
...
- `--version` — print the installed distribution's version and exit **0**. The one flag here that starts no mission and emits no record: it answers *what is deployed on this host*, read from the metadata the installer wrote — the same source `pip show` reads, because asking pip about a name and hoping the thing on `PATH` came from it is not an answer. A checkout being run in place says so in words (`judais-lobi (uninstalled checkout)`) rather than reporting a number out of the source tree, which would answer a different question than the one asked. Published here because a platform that pins a release has to be able to ask the binary which release it got.
...
- `JUDAIS_LOBI_OUTPUT_PROFILE` — the local backend's output budget when no numeric ceiling is configured: `standard` (default, 8,192 tokens), `synthesis` (16,384), or `extended` (32,768). Unknown or blank values select `standard`. The implicit budget is capped at half the configured or probed context capacity, leaving input room on smaller models; an unknown capacity is not reported as measured. The backend reads the profile at construction, so changing a later run's profile does not mutate another active run. This is endpoint configuration, not a CLI flag.
- `JUDAIS_LOBI_MAX_OUTPUT_TOKENS` — an explicit positive numeric output ceiling, taking precedence over the profile. Unset, blank, garbage or non-positive values select the profile instead. A per-call `max_tokens` remains authoritative. The effective ceiling is reserved before input fitting. Without an explicit numeric ceiling, a reported token-limit stop may raise the reserve within measured headroom, never beyond 32,768 or half the context capacity; it cannot raise an operator's explicit ceiling. Output budgets do not enlarge the model's context window.

A token-limited answer can continue for at most two additional text-only model calls, still within the run's step/wall-clock bounds and each request's output ceiling. Usable answer text is retained, including partial answer envelopes; incomplete tool arguments are never repaired into an executable action. Continuation does not repeat tools to regenerate prose. If recovery cannot finish, the result is a partial draft with an incomplete/budget outcome, not a complete answer or raw unfinished JSON. Normal grounding checks still apply to a completed answer. Detection currently uses the usage side channel's stop reason; absent usage/stop metadata is not inferred from text. This is in-run recovery, not a promise of process-restart continuation or a cumulative token-budget enforcer.
...
- `JUDAIS_LOBI_COGNITION` — the environment form of `--cognition`; any non-empty value turns the shadow store on. The flag wins.
- `JUDAIS_LOBI_COMPILED_CONTEXT` — the environment form of `--compiled-context`; any non-empty value puts the compiled view into each step's model input, and implies `JUDAIS_LOBI_COGNITION`. The flag wins.
- `JUDAIS_LOBI_SWARM_STEERING` — the environment form of `--swarm-steering`; any non-empty value offers a staged turn's planner the independent groups of what is still owed, and implies `JUDAIS_LOBI_COGNITION`. The flag wins.
- `JUDAIS_LOBI_GRAPH_CONTEXT` — the environment form of `--graph-context`; any non-empty value keeps the run's links as a topology in `graph.jsonl` and adds the RELATED section to the compiled view, and implies `JUDAIS_LOBI_COMPILED_CONTEXT`. The flag wins.
