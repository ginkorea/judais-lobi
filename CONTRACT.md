...
`mission_started`, and `usage` on `tool_call`, `answer`, `reply_rejected` and
`mission_finished`. `plan` rode `mission_started` until 0.8.x: that record is
now emitted before triage —
