...
|---|---|---|
| ~~**No sandbox by default.**~~ Closed 0.9.0: `select_sandbox` picks bwrap wherever it exists, the child env is an allow-list, and the choice rides `mission_started.sandbox`. **Closed for the in-process tools too:** bwrap isolates a subprocess and `FsTool` is pathlib in this interpreter, so a mission now carries a ROOT — its working directory, `core/tools/root.py` — and `fs`, `patch`, `git` and `repo_map` refuse a path resolving outside it (absolute, `..`, symlink) as an ordinary tool result. Chat is unrooted, as it was. **Still open:** nothing roots a *bridged* server's tools, which execute on the server. | `core/tools/sandbox.py`, `core/tools/fs_tools.py`, `core/tools/root.py` | 1 |
...
| 18 | Compiled context + grammar compiler | ✅ 1.3.0 (14 Sep 2026) — `--compiled-context` (the runtime's view in the model's input: facts with confidence bands + receipt handles, conflicts both sides, budgeted, never gating, oracle-verified cut) and `supports_json_schema` + extraction `--constrained`. The ablation ARM is live; the NUMBERS are Phase 19's to take |
| 19 | Obligation control — the core experiment | §2.9.6 — the commit-or-kill decision for the whole arc |
| 20 | Conditional expansions — graph working set, derived swarm, solver, routing | §2.9.7 — ✅ **complete in 1.5.0**: `--graph-context` (the graph core's context-feeding arm: `graph.jsonl`, hydrate-from-owed, RELATED section), `--swarm-steering` (the frontier's independent groups offered to a staged planner, advisory forever), pack `constraints:` (exact `Fraction` arithmetic; the solver an optional extra that buys checking, never loadability), and the registry (1.4.0). The measured ablation of every arm is the owner's formal benchmark on the reference platform, queued at this tag |
