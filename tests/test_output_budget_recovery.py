...
def exhausted(tokens=4096, prompt=18000, finish="length"):
...
def test_budget_recovery_and_context_reserve_match(monkeypatch):
    ...
    assert window.profile.max_output_tokens == 4096
    raised = client.recover_output_budget(exhausted())
    assert raised == 8192
    window.reserve_output_tokens(raised)
    assert local.output_bound == local.capabilities.max_output_tokens == 8192
    assert window.limit_tokens == 131072 - 8192
...
def test_recovery_has_headroom_and_a_finite_ceiling(monkeypatch):
    ...
    assert local.recover_output_budget(exhausted()) == 8192
    assert local.recover_output_budget(exhausted(8192)) == 16384
    assert local.recover_output_budget(exhausted(16384)) is None
...
def test_empty_length_is_retried_once_and_every_call_is_billed(monkeypatch, protocol):
    ...
    assert len(seen) == 2
    assert budget_at_call == [(4096, 4096), (8192, 8192)]
    ...
    assert [event["usage"]["completion_tokens"] for event in rejected] == [4096, 8192]
