...
- **prompt** `2c78315dfca7` (both turns, digested; the grammar is in the digest when one was sent)
...
- **commit** `9148adecc9fe629e9296ad84d17f72018a674270`
- **date** 2026-09-13
- **probes** `tests/fixtures/extraction/probes.jsonl` — 49 × 3 repeat(s) = 147 attempt(s)

**Every number below is true of `local` / `openai/gpt-oss-20b` @ temperature 0.2, endpoint `http://127.0.0.1:8011/v1`, decoding unconstrained, prompt `2c78315dfca7`, scorer 2, commit `9148adecc9fe`, 49 probe(s) × 3 repeat(s) and of nothing else.** A number without its interpreter beside it is not evidence — the 1.0.0 final gate is in `EVAL.md` §12, and this is that lesson written into the report rather than remembered.

## the number

| category | k/n (95% Wilson) | what one k is |
|---|---|---|
| `structural` | 147/147 = 100% [97%–100%] | replies that parsed as propositions, first try or after the one repair |
| `first_try` | 147/147 = 100% [97%–100%] | replies that parsed with no repair at all |
| `repair` | 0/147 = 0% [0%–3%] | attempts that needed the repair turn (LOWER is better) |
| `grounded` | 126/126 = 100% [97%–100%] | ASSERTs the receipt supports — the value under the field they name, quoted from a span really in it. The denominator is every ASSERT of every kind of probe, so a corpus with more assert probes in it moves this n |
| `gold_precision` | 121/122 = 99% [96%–100%] | ASSERTs that are a fact the probe asked for (assert and trap probes; an abstain probe asks for no fact, so its assertions are counted elsewhere) |
| `gold_recall` | 121/126 = 96% [91%–98%] | facts the probe asked for that were asserted AND grounded — a right value off an invented span is not a hit |
| `abstention` | 26/27 = 96% [82%–99%] | probes where SILENCE is the only right answer, answered with no assertion at all. Probes that declare a better alternative — a conflict to surface, a mask to transcribe — are not counted here |
...
| `trap` | 36/36 = 100% [90%–100%] | trap probes that did not ASSERT from the trap field |
| `hedged` | 0/63 = 0% [0%–6%] | attempts that touched what their probe was watching — a trap field, a key declared absent, one side of a conflict — at HYPOTHESIZE or AMBIGUOUS. Hedged wrongness, reported beside `trap` and NOT folded into any verdict |
| `probe` | 141/147 = 96% [91%–98%] | ATTEMPTS answered correctly and completely — every gold fact asserted AND grounded, nothing else asserted, the trap not taken, a conflict not swallowed, a mask not replaced. A correct answer with fabricated provenance is a failure |
| **`probe_reliable`** | 45/49 = 92% [81%–97%] | PROBES whose every attempt was right. The headline under --repeats: a gatekeeper is a question about reliability, and no majority voting |
...
|---|---|---|---|---|---|---|
| `present` | assert | 23/24 = 96% [80%–99%] | 71/72 = 99% [93%–100%] | 72/72 = 100% [95%–100%] | 89/89 = 100% [96%–100%] | — (nothing counted) |
| `absent` | abstain | 5/5 = 100% [57%–100%] | 15/15 = 100% [80%–100%] | 15/15 = 100% [80%–100%] | — (nothing counted) | 0/15 = 0% [0%–20%] |
| `contradiction` | abstain | 2/2 = 100% [34%–100%] | 6/6 = 100% [61%–100%] | 6/6 = 100% [61%–100%] | — (nothing counted) | 0/6 = 0% [0%–39%] |
| `masked` | abstain | 2/2 = 100% [34%–100%] | 6/6 = 100% [61%–100%] | 6/6 = 100% [61%–100%] | 3/3 = 100% [44%–100%] | — (nothing counted) |
| `cause_absent` | abstain | 2/2 = 100% [34%–100%] | 6/6 = 100% [61%–100%] | 6/6 = 100% [61%–100%] | — (nothing counted) | 0/6 = 0% [0%–39%] |
| `partial_coverage` | abstain | 1/2 = 50% [9%–91%] | 5/6 = 83% [44%–97%] | 6/6 = 100% [61%–100%] | 1/1 = 100% [21%–100%] | — (nothing counted) |
| `unit_semantics` | trap | 5/6 = 83% [44%–97%] | 15/18 = 83% [61%–94%] | 18/18 = 100% [82%–100%] | 15/15 = 100% [80%–100%] | 0/18 = 0% [0%–18%] |
...
| `runs_elapsed` | present | assert | 1 | first | 0 | — | 0/1 | silent | FAIL |  |
...
| `nodes_not_edges` | unit_semantics | trap | 1 | first | 0 | — | 0/1 | silent | FAIL |  |
...
| `shown_not_visible` | optional_filter | trap | 1 | first | 1 | 1/1 | 1/1 | asserted | PASS |  |
...
| `no_run_cost` | absent | abstain | 2 | first | 0 | — | — | silent | PASS |  |
...
| `why_job_failed` | cause_absent | abstain | 2 | first | 0 | — | — | silent | PASS |  |
...
| `only_matching_record` | partial_coverage | abstain | 2 | first | 0 | — | — | silent | PASS |  |
...
| `job_state_disagrees` | contradiction | abstain | 3 | first | 0 | — | — | both sides | PASS |  |
| `run_stage_disagrees` | contradiction | abstain | 3 | first | 0 | — | — | both sides | PASS |  |
...
| `shown_not_visible` | optional_filter | trap | 3 | first | 1 | 1/1 | 0/1 | asserted | FAIL |  |
